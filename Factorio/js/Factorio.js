class Factorio{
	...
	getElectricity(electricityTypeID){
		return this.data.electricity[electricityTypeID];
	}
	getFuel(fuelID){
		return this.data.fuel[fuelID];
	}
	getGenerator(generatorID){
		return this.data.generator[generatorID];
	}
	getAccumlator(accumlatorID){
		return this.data.accumlator[accumlatorID];
	}
	getModule(moduleID){
	...
	getIcon(type, id, checkItem=true){
		...
		var val = this.data[type][id];
		if(val.hasOwnProperty('icon') && val['icon']) return 'img/icon/'+val['icon'];
